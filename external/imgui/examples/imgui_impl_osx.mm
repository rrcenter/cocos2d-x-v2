...
//        io.DisplayFramebufferScale = ImVec2(dpi, dpi);
        ...
        auto factor = [[EAGLView sharedEGLView] getFrameZoomFactor];
        io.DisplayFramebufferScale = ImVec2(factor, factor);
...
{
    float scale = 1.0f;
    scale = [[EAGLView sharedEGLView] getFrameZoomFactor];
    ...
    if (event.type == NSEventTypeMouseMoved || event.type == NSEventTypeLeftMouseDragged)
    {
        ...
        io.MousePos = ImVec2(mousePoint.x / scale, mousePoint.y / scale);
    }
